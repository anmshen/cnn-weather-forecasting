...
if os.path.exists(NORM_CACHE):
    ...
    cont_mean, cont_std = norm.get('cont_mean'), norm.get('cont_std')
    if cont_mean is None or cont_std is None:
        raise RuntimeError('norm_stats.pt exists but missing cont_mean/cont_std')
else:
    ch_mean, ch_std = compute_norm_stats(train_idx)
    torch.save({'mean': ch_mean, 'std': ch_std,
                'cont_mean': cont_mean, 'cont_std': cont_std}, NORM_CACHE)
