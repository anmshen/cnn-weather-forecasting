...
print(f"Target shape: {target_vals.shape}")
print(f"Binary label shape: {target_bin.shape}")

# ── Normalize targets ────────────────────────────────────────────
clean = torch.nan_to_num(target_vals[train_idx], nan=0.0)
...
print(f"Mean: {cont_mean}")
print(f"Std:  {cont_std}")
...
print("\n── Input Normalization ──────────────────")
print(f"Channel mean shape: {ch_mean.shape}")
print(f"Channel std shape:  {ch_std.shape}")
print(f"First 5 means: {ch_mean[:5]}")
print(f"First 5 stds:  {ch_std[:5]}")
...
class WeatherDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx == 0:
            print(f"Input shape: {x.shape}")   # should be (C, H, W)
            print(f"Cont target shape: {y_cont.shape}")  # (6,)
            print(f"Binary target: {y_bin}")
